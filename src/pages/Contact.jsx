...
const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error("Erreur création demande :", err.response ? err.response.data : err.message);
      setError("Erreur lors de l'envoi de la demande. Vérifie la console pour plus de détails.");
    }
  };
    ...
    <div className="min-h-screen bg-zinc-900 relative overflow-x-hidden pt-20 sm:pt-24">
      ...
      <div className="flex justify-center items-start sm:items-center min-h-[calc(100vh-5rem)] px-4 sm:px-6 md:px-8 lg:px-12 py-8 sm:py-12">
        <div className="w-full max-w-md sm:max-w-lg md:max-w-xl lg:max-w-2xl min-h-[500px] sm:min-h-[600px] md:h-[700px] bg-zinc-800 rounded-lg p-4 sm:p-6 md:p-8 flex flex-col">
          ...
          {success && (
            <p className="text-green-400 mb-3 sm:mb-4 text-sm sm:text-base text-center">{success}</p>
          )}
          {error && (
            <p className="text-red-500 mb-3 sm:mb-4 text-sm sm:text-base text-center">{error}</p>
          )}
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3 sm:gap-4 flex-1">
            ...
            <div className="flex-shrink-0">
              <label className="text-white text-sm sm:text-base mb-1 sm:mb-2 block">Objet</label>
              <input
                type="text"
                name="subject"
                value={form.subject}
            ...
            <div className="flex-1 min-h-[200px] sm:min-h-[250px] md:min-h-[300px]">
              <label className="text-white text-sm sm:text-base mb-1 sm:mb-2 block">Message</label>
              <textarea
                name="message"
                value={form.message}
                onChange={handleChange}
