...
const EventsComing = ({ onSelectEvent }) => {
  ...
  if (loading)
    return <div className="text-white text-center py-6">Chargement...</div>;

  const getTabClass = (tab) =>
    `px-4 py-1 text-sm font-semibold rounded transition-colors duration-200 cursor-pointer ${
      activeTab === tab
    ...
    <div className="flex flex-col gap-4">
      ...
      {Object.entries(eventsBySport).map(([sport, sportEvents]) => (
        <div key={sport} className="flex flex-col gap-2">
          {/* Nom du sport */}
          <div className="text-orange-400 font-semibold uppercase text-sm mb-1">
            {sport}
          </div>
