...
const ScoreBoardDetails = ({ event, onClose }) => {
  ...
  return (
    <div className="w-full bg-zinc-900 rounded-xl p-6 shadow-lg flex flex-col gap-6 text-white">
      ...
      {/* Type de sport */}
      <div className="text-orange-400 text-lg font-semibold text-center">
        {event.type || "Sport inconnu"}
      </div>
        ...
        <div className="flex-1 text-center flex flex-col items-center">
          {isLive ? (
            <>
              ...
              <span className="text-2xl font-mono mt-2 text-white/80">
            ...
            <>
              <span className="animate-pulse text-orange-500/90 text-xl font-bold mt-2">
                Match à venir
              </span>
              {timeToStart !== null && (
            ...
            {eventDate && (
              <div className="text-gray-300 text-sm">
                ...
                <span className="text-orange-400">
                  {eventDate.toLocaleDateString("fr-FR")} à{" "}
        ...
        {activeTab === "tournoi" && (
          <div className="bg-zinc-700 p-4 rounded-lg border-l-4 border-orange-400 flex flex-col gap-2">
            <div className="text-orange-400 font-semibold">
              Tournoi / Événements associés
            </div>
            {event.tournament?.matches?.map((match, idx) => (
              <div key={idx} className="text-white text-sm pl-2">
                • {match.title} -{" "}
                {new Date(match.dueDate).toLocaleDateString("fr-FR")}
              </div>
            ))}
            {!event.tournament && (
              <div className="text-gray-400 text-sm">Aucun tournoi associé</div>
            )}
          </div>
        )}
      </div>
